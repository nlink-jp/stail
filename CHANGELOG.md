...
Includes every change listed under 0.5.1, which was never published.
...
- **darwin code-signature identifier** is now the canonical `stail`
  (was `stail-darwin-arm64`), set via `codesign -i` so it stays stable after
...
- **Darwin releases are now Developer ID signed and Apple-notarized.**
  `stail-v0.4.1-darwin-{amd64,arm64}.zip` carry full Apple Developer
  ...
  users who place `stail` under Dropbox-synced (or any other
